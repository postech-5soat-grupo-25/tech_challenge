...
impl UsuarioUseCase {
    ...
    pub async fn create_usuario(
        ...
        let valid_tipo: Tipo = Tipo::from_str(usuario.tipo.as_str()).unwrap();
        let valid_status: Status = usuario.status.parse().unwrap();
        let _now = Utc::now().format("%Y-%m-%d %H:%M:%S%.3f%z").to_string();

        // TODO: passar apenas os campos necessarios, sem incluir id e datas, status
    ...
    // TODO: definir um input mais adequado para update
    pub async fn update_usuario(
        ...
        let valid_cpf = Cpf::new(usuario.cpf.clone())?;
        let valid_tipo: Tipo = Tipo::from_str(usuario.tipo.as_str()).unwrap();
        let valid_status: Status = Status::from_str(usuario.status.as_str()).unwrap();
        let _now = Utc::now().format("%Y-%m-%d %H:%M:%S%.3f%z").to_string();
